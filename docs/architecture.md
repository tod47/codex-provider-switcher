...
- Provider：EndpointPreflight 检查 HTTP/HTTPS endpoint、responses wire API 和凭据是否存在，并用 HEAD 做轻量可达性探测。
- Process：ChatGPTProcessController 识别已运行的 ChatGPT/Codex app-server，优雅退出、等待停止，再以受控环境重新启动。
- Switch：SwitchTransactionCoordinator 串联锁、快照、停止、原子写入、启动、完成和回滚。
...
如果 ChatGPT 未能停止，事务在原子替换前终止；原配置和历史 sentinel 应保持不变。锁使用一个 app-support 目录下的原子创建目录，避免两个切换同时进行。
...
配置转换保留 model_provider = "custom" 标识，目的是不主动创建新的历史 bucket。快照文件放在应用自己的 snapshots/ 子目录，manifest 记录当前模式、最近 GPT 快照和事务阶段。
...
代码使用 FileManager.homeDirectoryForCurrentUser、应用支持目录和参数注入，不嵌入某个开发者的个人路径。不同 macOS 用户、ChatGPT 安装位置、客户端 bundle identifier 或 Codex 配置版本可能需要通过参数或后续适配支持。
